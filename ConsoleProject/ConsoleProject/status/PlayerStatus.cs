...
namespace ConsoleProject.status
{
    public class PlayerStatus : Status
    {
        public override void ShowStatus()
        {
            ...
            Util.Print($" 공격력 : ");
            Util.Print($"{GameManager.player.Damage + GameManager.player.equipWeapon.Damage,5}", ConsoleColor.Magenta);
            Util.Print($" 힘     : ");
            ...
            Util.Print($" 방어력 : ");
            Util.Print($"{(int)GameManager.player.Defense + GameManager.player.equipArmor.Defense,5}", ConsoleColor.DarkBlue);
            Util.Print($" 민첩   : ");
